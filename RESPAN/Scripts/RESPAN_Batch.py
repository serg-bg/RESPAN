...
#RESPAN install directory to path
import sys
import os
import logging
...
# when log messages contain non-ASCII chars (mu, arrows, etc.).
try:
...
# Ensure the repo root (parent of RESPAN/) is on sys.path so the
# `from RESPAN.Environment import *` below resolves regardless of cwd.
_repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _repo_root not in sys.path:
    sys.path.insert(0, _repo_root)

#import RESPAN modules
...
data_dirs = ["D:/Project_Data/RESPAN/Figure5_SBG/"]

#data_dirs = ["D:/Project_Data/RESPAN/Testing/Spine_tracking/"]
 #            ] #,
    ...
    if os.path.isdir(data_dir):
        for subfolder in os.listdir(data_dir):
            ...
            if os.path.isdir(subfolder_path): 
                ...
                logname = subfolder_path+'RESPAN_Log_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") +'.log'
                ...
                logger.info("RESPAN Version: "+__version__)
                ...
                #settings.seg_model_path = "D:/Dropbox/Github/spine-analysis/RESPAN/Models/Dataset022_SpinesDendrites/"

...
def _aug_complete(src_dir, tgt_dir, n_src, n_rot):
    ...
    return (
            len(src_files) == len(tgt_files) == expected and
            src_files == tgt_files
    )
...
def train_care(inputdir, model_output, model_name, augmentation, image_type, patch_size,
    ...
    if augmentation:
        ...
        # ── check status ─────────────────────────────────────────────────────
        degrees_list = [90, 180, 270]
        n_original_tif = len([f for f in os.listdir(os.path.join(inputdir, 'lowSNR'))
                              if f.lower().endswith('.tif')])

        if _aug_complete(output_augmented_dir, output_high_snr_dir,
                         n_original_tif, len(degrees_list)):
            logger.info("CARE augmentations complete – skipping regeneration. "
            ...
            augment_images_v1(
                ...
                logger, is_low_snr=True, choices=[None]
            )
            augment_images_v1(
                os.path.join(inputdir, 'highSNR'),
                output_high_snr_dir,
                logger, is_low_snr=False, choices=[None]
            )
    # Prepare data
    ...
    # Training
    (X, Y), (X_val, Y_val), axes = load_training_data('CAREdata/my_training_data.npz', validation_split=pct_validation,
                                                      verbose=True)
